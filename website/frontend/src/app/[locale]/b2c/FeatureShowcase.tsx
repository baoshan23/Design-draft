 ...
 * area. All panels are rendered (SSR-friendly) and toggled via `hidden`
 * so the marketing copy stays crawlable. Tab bar is an ARIA tablist
...
export default function FeatureShowcase({ panels }: { panels: Panel[] }) {
  ...
  return (
    <div className="b2c-showcase">
      <div className="b2c-showcase-tabs" role="tablist" aria-label="Product features">
        {panels.map((p, i) => (
          <button
            key={p.id}
