...
export default function DemoTabs() {
  ...
  const tabs = [
    ...
    { id: 'demo-access', label: t('product.demo.tab.access') },
    { id: 'demo-credentials', label: t('product.demo.tab.credentials') },
  ];
  ...
  return (
    <TabSwitcher tabs={tabs}>
      ...
      <div id="demo-live">
        <div className="grid grid-2" style={{ gap: 20 }}>
          <div className="card" style={{ textAlign: 'center', padding: 32 }}>
            ...
            <h4 style={{ marginBottom: 8 }}>{t('product.demo.admin.title')}</h4>
            <p style={{ fontSize: '0.85rem' }}>{t('product.demo.admin.desc')}</p>
            <a href="/product" className="btn btn-primary btn-sm" style={{ marginTop: 16 }}>{t('product.demo.admin.btn')}</a>
          </div>
          <div className="card" style={{ textAlign: 'center', padding: 32 }}>
            ...
            <a href="/product" className="btn btn-accent btn-sm" style={{ marginTop: 16 }}>{t('product.demo.merchant.btn')}</a>
          </div>
        </div>
        ...
        <div className="grid grid-2" style={{ gap: 20, marginTop: 20 }}>
          ...
          <div className="card" style={{ textAlign: 'center', padding: 32 }}>
            <h4 style={{ marginBottom: 16 }}>{t('product.demo.h5.title')}</h4>
            <div className="qr-placeholder" style={{ margin: '0 auto' }}>QR Code</div>
            <p style={{ fontSize: '0.8rem', marginTop: 12 }}>{t('product.demo.h5.desc')}</p>
          </div>
        </div>
      </div>
      ...
      <div id="demo-access">
        <div className="card" style={{ padding: 32 }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
            {[
              { label: t('product.demo.access.admin'), url: 'https://demo-admin.gcss.cloud' },
              ...
              <div key={link.url} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '12px 16px', background: 'var(--gray-50)', borderRadius: 'var(--radius)', border: '1px solid var(--gray-200)' }}>
                ...
                <a href="/product" className="btn btn-sm btn-outline">{t('product.demo.access.visit')}</a>
              </div>
            ))}
          </div>
        </div>
      ...
      <div id="demo-credentials">
        <div className="grid grid-2" style={{ gap: 20 }}>
          <div className="card" style={{ padding: 32 }}>
            <h4 style={{ marginBottom: 16 }}>{t('product.demo.cred.admin')}</h4>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
              <div style={{ padding: '10px 14px', background: 'var(--gray-50)', borderRadius: 'var(--radius)', border: '1px solid var(--gray-200)' }}>
                ...
                <div style={{ fontFamily: 'var(--font-mono)', fontWeight: 600, fontSize: '0.9rem' }}>admin</div>
              </div>
              <div style={{ padding: '10px 14px', background: 'var(--gray-50)', borderRadius: 'var(--radius)', border: '1px solid var(--gray-200)' }}>
