...
- 2026-05-19 — **b2c (`/[locale]/b2c`) hero shot illustration swap — REDONE via a dedicated new filename (root cause: a concurrent session's commit reverted the in-place file)** (commit `4da2c5b`, pushed fork `design-draft-local` per PUSH POLICY, no force; `./node_modules/.bin/tsc --noEmit` CLEAN). User: "没有改到，重新改" (the prior in-place swap `e1dcb38` showed no change). ROOT CAUSE diagnosed via `git log -- public/images/b2c-hero-monitor.png`: my `e1dcb38` overwrote `b2c-hero-monitor.png` (725623→1505810 B) but a LATER concurrent-session commit **`a145cac`** (`fix(b2b): bump .product-sub-nav bg…`) reverted that same file back to the ORIGINAL 725623-B monitor PNG — so at HEAD the JSX still pointed at `b2c-hero-monitor.png` whose bytes were the old monitor again → user saw no change. FIX = decouple via a UNIQUE filename a concurrent session won't touch: (1) restored `b2c-hero-monitor.png` to its original (`git checkout e1dcb38^ --`, now unused, kept intact); (2) saved the new 2000×2000 RGBA art as a NEW file `public/images/b2c-hero-illustration.png` (1.5 MB); (3) **`b2c/page.tsx` only**, hero `<Image src="/images/b2c-hero-monitor.png"` → `src="/images/b2c-hero-illustration.png"` (the `width={2000} height={2000}` from `e1dcb38` survived at HEAD, kept; `alt/priority/sizes/style` unchanged). Same CSS path as before (`.hero-shot` no frame; `.product-hero .hero-shot img` `max-height:40vh;object-fit:contain` → square art centered, ratio-correct). tsc CLEAN. Verified HEAD: JSX src = new file, `b2c-hero-illustration.png`=1505810 B tracked, `b2c-hero-monitor.png`=725623 B (original, orphaned). LESSON: when a concurrent session is active, do NOT swap images in place — a same-path revert by another commit silently undoes it; use a fresh filename + repoint the src. To revert: point src back to `/images/b2c-hero-monitor.png` (still the original monitor) + optionally `git rm` the new file. **NOT VISUALLY VERIFIED in-browser** — robust-by-construction (unique path immune to the collision that broke `e1dcb38`). ───── PRIOR: **b2b/product/b2c二级导航 (`.product-sub-nav`) 背景白色不透明度 0.10 -> 0.92 -> 0.74 -> 0.65** (commits `a145cac` 0.10->0.92, then `ea84ecd` 0.92->0.74, then `pending` 0.74->0.65; pushed fork `design-draft-local` per PUSH POLICY, no force; CSS-only -> no tsc). User: "这里的红框的二级导航的背景，白色透明度提高一点" + screenshot (red box = the b2b `.sub-nav` strip under the header, blue hero + orange CTA bleeding through). ROOT CAUSE: the b2b `SubNav.tsx:40` renders `<nav className="sub-nav product-sub-nav">`. The prior site-wide nav-frost unification set base `.sub-nav` (sections.css:4097) to `rgba(255,255,255,0.92)` but **MISSED `.product-sub-nav`** (`pages-extracted.css:4`, `background:rgba(255,255,255,0.10)`); equal specificity (0,1,0) but pages-extracted.css loads AFTER sections.css so the 0.10 won -> near-transparent bar on every `.product-sub-nav` page (product/b2b/b2c). FIX = **`pages-extracted.css` only**, the `.product-sub-nav` block (~10-17): light `rgba(255,255,255,0.10)` -> `0.92` -> `0.74` -> `0.65`, dark mirror `rgba(12,10,9,0.10)` -> `0.92` -> `0.74` -> `0.65`, `blur(28px)` -> `blur(24px)` (both -webkit + std) to exactly match the unified base `.sub-nav`/header frost. `border-bottom:1px solid var(--border-subtle)` + the `position:fixed!important` block untouched. Now consistent with the rest of the site (header + base sub-nav all 0.92 opaque frost). CSS-only, no JSX/i18n -> no tsc. To revert: restore `0.10`/`0.10` + `blur(28px)`. Final value = **0.65** (user found 0.92 too opaque). **NOT VISUALLY VERIFIED in-browser** (autodebug needs user preview) — robust-by-construction (completes the proven prior 0.92 frost unification on the one selector it missed). ----- PRIOR: Homepage `#how-it-works` "3 步快速上线" 3 steps got a CSS-only staggered scroll-reveal + hover animation (commit `94d3a58`); revert by deleting the appended "staggered scroll-reveal + hover interaction" block in `pages.css` after `.hiw-card p` (~3266) and restoring base `.hiw-card` to no transition.
