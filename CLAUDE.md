...
- 2026-05-12 (mobile menu simplification): Removed the separate mobile footer/utility block and moved only **Log In** / **Sign Up** to the top of the mobile nav above Home. **Header.tsx** now renders a compact auth row at the start of the menu; **pages.css** replaces the old footer styles with smaller, cleaner top-row auth buttons. Verified in the browser and with `npm run build` (100 static pages generated). Commit `50b8f5a` pushed to `main` on `anindya127/Design-draft`. No production deploy per policy.
