...
- 2026-04-24 (cross-page linking + light-mode cleanup): Reworked 3D orbs → 2D blurred bouncing balls (pure DOM + raf, gold radial gradients, dropped three.js deps usage). Fixed theme selector bug — `ThemeProvider` removes the `data-theme` attribute in light mode rather than setting `="light"`, so all `[data-theme='light']` selectors were dead CSS; rewrote 23 of them to `html:not([data-theme='dark'])` in polish.css + sections.css. Suppressed every site-wide decorative gold gradient in light mode (`.hero`, `.hero::before`, `.particles-bg::after`, `.mesh-bg::before`, `.hero-image-placeholder::after`, `.auth-visual::before`, `.product-hero::before`, `.feature-image-placeholder::after`, `.feature-row::before`, `.glass-card.tilt-card::before`, `.cta-section::before`, `#gallery::before/::after`, `.language-cloud::before`, `.card::before`, `.model-card::before`, `.section-alt::after`, `.admin-bg`, `.orbs-root`). Dark mode orbs/glass preserved. Removed `.section-header::before/::after` decorative corner brackets. Bumped text-tertiary/muted tokens in both themes for AA small-text contrast. **Navigation pass:** added Header `.btn-buy` primary CTA (→ /buy), demoted `.btn-demo` to outlined secondary, added `.btn-signup` next to login for guests, mobile-nav footer gained Buy + Sign Up. Footer: added Buy Now to Product col, new 5th "Account" column with Log In / Sign Up / Dashboard / Invoices. **Cross-links:** new `components/sections/RelatedPages.tsx` with 14 target keys; wired a 4-card "Keep exploring GCSS" grid onto Home, B2B, B2C, Pricing, Contact, FAQ, Forum, Blog list, Docs, About, Careers, Partners — each page now surfaces 4 peer destinations. Full `related.*` i18n namespace in EN + ZH. Build green. Multiple deploys (7m 7s → 10m 42s → 8m 52s → 7m 46s).
