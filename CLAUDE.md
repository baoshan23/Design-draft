...
- 2026-05-30 — **Homepage `#models` "Flexible Operating Models" section background → white** (`pages.css` line 3167, CSS-only, commit `7af5764` pushed fork `design-draft-local`). The Business-Models band (between the "Compatible with leading EV charger manufacturers" logo strip and the "Native iOS & Android app" block) rendered on the light-grey `var(--dark)` = `#F1F2F4` (from the generic `.section-alt{background:var(--dark)}` rule at `pages.css` 1247). Same file already had an override for `#how-it-works.section-alt, #features.section-alt{background:#fff}` at line 3167 — added `#models.section-alt` to that selector list, no new rule. Markup unchanged: `BusinessModelsSection.tsx` still uses `<section className="section section-alt" id="models">`. Revert: drop `#models.section-alt` from the comma-list at `pages.css` 3167.
