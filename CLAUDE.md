...
- 2026-05-12 (mobile nav cleanup + menu optimization): Moved the mobile-only footer actions into the main navigation panel as a compact `mobile-nav-utilities` block, removing the separate footer feel and tightening spacing/typography for tablet/mobile. **Header.tsx** now keeps Buy/Demo/login/signup/language/theme controls inside the main menu flow; **pages.css** adds compact card-like styling for the embedded quick actions and smaller touch targets to reduce visual weight. Verified in the browser on tablet/mobile widths, menu open/close and dropdown interactions work, and `npm run build` passes with 100 static pages generated. Commit `7712f3a` pushed to `main` on `anindya127/Design-draft`. No production deploy per policy.
