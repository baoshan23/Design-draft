...
- 2026-05-12 (CSS vendor prefix standardization + inline style removal + ARIA fixes): **Comprehensive frontend linting pass** to fix all prefix warnings and style issues. **CSS Fixes**: pages.css — added `-webkit-user-select` prefixes (`.admin-legacy-group`, `.switch`), fixed vendor ordering in buy-summary backdrop-filter, guarded unsupported `overscroll-behavior: contain` with comment; sections.css — fixed 5 backdrop-filter vendor orderings (hero-footer-bar, .float-badge, .pricing-card, .sub-nav, card/glass-card), added `-webkit-hyphens` prefix to .comparison-table, added `-webkit-user-select` to hero-art-type, guarded `overflow-x: clip` with `@supports` for iOS < 16, deprecated `-webkit-overflow-scrolling: touch` commented out in 2 places. **Inline Style Conversions** (30+ instances in page.tsx): created new `utilities.css` with 20+ reusable classes (.mb-16, .h-full, .border-light, .text-primary-color, .matrix-item-highlight, .matrix-platform-bg, .text-center, .mt-24, .bg-dark, .font-semibold, .text-sm-badge, 9 language cloud font-size utilities). Converted all inline styles in card grids, matrix columns, language cloud, and CTA section to utility classes. **ARIA Accessibility**: Header.tsx — converted dynamic `aria-expanded` and `aria-pressed` values to string literal ternaries, added `role="menuitem"` to 6 dropdown menu items; AppSlideshow.tsx — converted dynamic `aria-hidden` and `aria-selected` to string literals. Remaining linter warnings (8 Header.tsx, 2 AppSlideshow.tsx) are **false positives** — ternary operators correctly evaluate to valid string values at runtime per ARIA spec. Build verified: 100 static pages generated. Commit b606c6e pushed to fork (anindya127/Design-draft). No production deploy per policy.
