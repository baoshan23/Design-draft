...
- 2026-05-21 — **b2c (`/[locale]/b2c`) `#gallery` "Product Gallery": replaced the flat 4-phone screenshot row with an interactive 3D perspective phone cluster** (new file `Gallery3D.tsx` + `b2c/page.tsx` + `polish.css`; `tsc` 0). User pasted a before-screenshot (4 flat phone screenshots in a `grid grid-4` row) and a 3D angled-mockup reference, asked to convert to that style + add interaction. Via AskUserQuestion the user chose: **keep all 4 screenshots** in a 3D cluster (2 back standing + 2 front leaning) and **both** interactions (mouse parallax + hover pop).
  ...
  ───── EARLIER (separate concurrent thread) — **Pricing page (`/[locale]/pricing`) comparison-table yellow `<thead>` polish** (`sections.css`, CSS-only, all pushed fork `design-draft-local`): (1) `7765016` made the header sticky on ≥1040px viewports — `.comparison-table-wrap .comparison-table{overflow:visible}` + `@media(min-width:1040px){.comparison-table-wrap{overflow:clip}}` (the `overflow-x:auto` wrap was a scroll container trapping the pre-existing `position:sticky` th). (2) `725253a` th `padding-bottom 36→33px`. (3) Stale-dev-server incident: user "没有改到" — the `next dev` on :8000 had a dead file-watcher serving a frozen CSS bundle; FIXED by kill + `rm -rf .next` + restart `npm run dev` (now in this session's background on :8000). (4) `42b84c7` header text centred + corners squared: `.comparison-table th` `padding 16/33→24/24` + `vertical-align top→middle`; `.comparison-table-wrap` `border-radius → 0 0 var(--radius-lg) var(--radius-lg)` (top corners square). (5) `c247b10` — user "功能两个字与左边文字左对齐" → restored `text-align:left` on the shared `.comparison-table td:first-child, th:first-child` rule so the `功能` header cell left-aligns with the body feature-name column (plan name/price header cells stay centred via the base `.comparison-table th,td{text-align:center}`). Revert (5): drop `text-align:left` from that rule. **VERIFIED in served dev CSS, not screenshot-checked.**
