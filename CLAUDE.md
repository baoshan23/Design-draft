...
- 2026-05-18 — `54d4432`: converted the 4 b2c feature-highlight blocks (精美 UI / 您的品牌 / APP·H5 / 智能运营中心 — i18n `product.ui|brand|app|ops.*`, were 4 `<ScrollAnimation>`-wrapped `feature-row`/`feature-row reverse` rows in `b2c/page.tsx` ~260-408) into an AstroPay-style **sticky stacking deck**. NEW `b2c/StackCards.tsx` (client): renders `<div className="b2c-stack">`, on rAF-throttled scroll/resize shrinks+dims each pinned card as the NEXT card rises to the sticky line — `p = clamp(1-(nextTop-stickTop)/(cardH*0.9),0,1)`, `scale 1→0.92`, `brightness 1→0.95`, `opacity 1→0.82`; no-ops on `prefers-reduced-motion` or `max-width:900px` (resets inline styles). `stickTop = --header-height(72) + 28`. page.tsx: 4 `<ScrollAnimation><div marginTop>…</div></ScrollAnimation>` → one `<StackCards>` with 4 `<article className="b2c-stack-card">` (inner `feature-row`/`.reverse` + AppSlideshow + dark smart-ops box ALL kept verbatim; zero i18n change, EN/ZH untouched). `ScrollAnimation` import still used lower in the file (Payment Matrix etc.). CSS in `sections.css` just above the `.b2c-feature-lined` comment (~375): `.b2c-stack` flex-col gap28 mt80 pb8vh; `.b2c-stack-card` `position:sticky; top:calc(var(--header-height,72px)+28px)` white radius28 padding `clamp(28px,4vw,64px)` soft shadow, `transform-origin:center top`, 120ms transition, `will-change:transform`; `.b2c-stack-card .feature-row{padding:0}` (card owns the padding); dark mirror `#15110b`; reduced-motion kills the transition; ≤900px → `position:static` plain stacked cards (radius20/pad24, JS also no-ops). `.b2c-stack*` is b2c-only — no bleed; `.feature-row` reused for the 2-col layout but the polish.css gold `::before` divider only matches sibling `.feature-row`s so wrapping each in its own `<article>` suppresses it (intended). `npx tsc --noEmit` CLEAN. **NOT VISUALLY VERIFIED** (autodebug needs user preview+debug); the sticky stack is pure CSS (universal), the shrink is JS progressive-enhancement — robust by construction. If revisiting: tune scale/opacity deltas or the `*0.9` range in StackCards.tsx; `pb8vh` gives the last card scroll room (raise if it ends too abruptly). PRIOR `0ea06c7`: removed the b2c "强大功能" band top/bottom gradient bands (now plain `border-top`/`border-bottom` 1px rgba(0,0,0,0.09), dark .1). PRIOR `4b099b4`: b2c "强大功能" cells → Moss stat-band look (`.b2c-feature-cell` icons hidden, big `clamp(1.9rem,2.6vw,2.5rem)` h3 w/ green `::before "+"`, tiny uppercase caption `p`). PRIOR `91f0b4e`/`8abb264`/`d045cde`: earlier b2c "强大功能" full-bleed lined-grid + restyle + removed UI-custom two-card block (see git log). PRIOR (context): b2c "系统演示" restyled to /demo shell (`b2c/DemoTabs.tsx` + `.b2c-demo-*` in `sections.css`, b2c-only replica).
