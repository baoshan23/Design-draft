...
- 2026-05-19 — **b2c (`/[locale]/b2c`) `#features` band “现代化 EV 充电的强大功能”: full-bleed lined band -> discrete card grid** (committed + pushed fork `design-draft-local` per PUSH POLICY, no force; CSS-only -> no tsc). User: "将这里的布局改成卡片形式展示" + screenshot (red box = the 6-feature `.b2c-feature-lined` block, ff1–ff6). **`src/app/styles/sections.css` only**, all `.b2c-feature-*` (b2c-only, single-sourced ~380+). (1) `.b2c-feature-lined`: dropped the full-bleed trick (`left/right:50%; width:100vw; margin-±50vw`) + top/bottom hairlines -> now `display:flex; flex-direction:column; gap:24px` inside `.container`. (2) `.b2c-feature-row`: dropped the content-box `padding-inline:max(…)` re-align + the `+row border-top` hairline; added `gap:24px` (still `grid repeat(3,minmax(0,1fr))`). (3) `.b2c-feature-cell`: removed `border-right`/`:first-child border-left` hairlines + `background:none`; now a card -> `background:var(--dark-card); border:1px solid var(--border-subtle); border-radius:var(--radius-lg); box-shadow:var(--shadow-sm); padding:34px 30px`; hover changed from bg-tint to `translateY(-4px)+shadow-md+border-color`. (4) Dark overrides slimmed: card bg/border are theme-aware tokens so only kept `:hover border-color` + the existing h3/p/icon dark rules (border/lined/row dark overrides deleted). (5) Responsive: ≤900 -> 2-col, ≤600 -> 1-col `padding:28px 22px`; removed the ≤600 single-col `+cell` hairline pair + the ≤480 `padding-inline:16px` (both moot off full-bleed). Icon grid (icon|h3/p) + `.b2c-feature-head` untouched. Brace balance verified ({ == } == 829). To revert: `git show <prev>:…/sections.css` for the `.b2c-feature-*` block (restores full-bleed lined band). FOLLOW-UP (same session, user: "去掉卡片边缘线"): `.b2c-feature-cell` `border:1px solid var(--border-subtle)` -> `border:0`, dropped `border-color` from transition + the `:hover border-color` (light) and the `[data-theme=dark] :hover border-color` rule — cards are now shadow-only (`--shadow-sm`, hover `--shadow-md`+lift). FOLLOW-UP 2 (same session, user: "卡片参考这种布局" + ref screenshot = icon-tile-left / title+↗ / desc): `sections.css` `.b2c-feature-*` only — (a) `.b2c-feature-cell` `column-gap 16->20`, `row-gap 10->8`, `padding 34px 30px -> 30px 32px`; (b) `.feature-card-icon` plain 36px -> **tinted rounded tile** `52×52`, `border-radius:14px`, `background:rgba(254,191,29,0.14)` (gold brand tint, NOT the ref's lavender — kept GCSS palette), `grid-row:1/3; align-self:center` (spans both text rows, vertically centred), svg `32->26`; dark `:feature-card-icon` `background:none` -> `rgba(254,191,29,0.16)`; (c) `h3` -> `display:flex; align-items:center; gap:8px` + new `h3::after{content:"\2197"}` link-arrow (CSS-only, no JSX/i18n) muted `#9aa0a8`, on `:hover` nudges `translate(2px,-2px)` + `var(--gcss-gold,#d69e00)`. FOLLOW-UP 3 (same session, user: "去掉卡片投影，保留原来边缘线，鼠标移上去才有投影，参考其他卡片；去掉所有箭头图标"): `sections.css` `.b2c-feature-cell` `border:0`->`border:1px solid var(--border-subtle)`, `box-shadow:var(--shadow-sm)`->`box-shadow:none` at rest, `:hover` keeps `translateY(-4px)+var(--shadow-md)` (= the site-wide border-rest / shadow-on-hover card pattern, e.g. `.b2b-arch-box`). DELETED the `h3::after`/`:hover h3::after` `\2197` arrow rules + reverted `h3` to block (removed `display:flex;align-items:center;gap:8px`). Icon-tile rules (FOLLOW-UP 2) unchanged. FOLLOW-UP 4 (same session, user: "图标色块改成黄色，线性图标改成黑色"): `.feature-card-icon` `background` `rgba(254,191,29,0.14)` -> **solid `var(--primary,#febf1d)`** (and dark override `rgba(254,191,29,0.16)` -> `var(--primary,#febf1d)`; `--primary` = #FEBF1D light / #E6A817 dark). Added a CSS-only stroke override `.b2c-feature-cell .feature-card-icon svg path/circle/rect/line{stroke:#181818}` — CSS `stroke` beats the inline two-tone `stroke="#181818"`/`"#FEBF1D"` attrs so all line icons are monochrome black (NO JSX/i18n change). Brace balance re-verified (829/829). **NOT VISUALLY VERIFIED in-browser** — robust-by-construction (token-driven card, b2c-only selectors, DOM unchanged). ───── ALSO (same session, user: "这里的'一站式体验'与'GCSS两大版本'间距大一点点" + /demo hero screenshot): `website/frontend/src/app/[locale]/demo/demo-page.css` `.demo-hero-title` (flex-column wrapping `.demo-hero-title-script` lead + `.demo-hero-title-main`) `gap:4px -> 10px` (single change, +6px). Demo-only file; revert: gap back to 4px. **NOT VISUALLY VERIFIED in-browser**.
