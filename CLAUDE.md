...
- 2026-05-21 — **b2c (`/[locale]/b2c`) `#multilingual` yellow panel: language-request form fixes** (`sections.css`, CSS-only, all in `.b2c-lang-aside .lang-request-card` rule ~3087, pushed fork `design-draft-local`).
  - (a) `5dfccb8` — **killed ghosted / double-rendered form text.** User red-boxed a faint smaller offset copy of the `需要我们尚未支持的语言？` form title overlapping itself. Root cause: the form is `<div class="card lang-request-card">` and global `.card` (~`sections.css` 2376) applies `backdrop-filter:blur(20px) saturate(1.2)`; the `.b2c-lang-aside` override made the card chromeless (transparent/no border/no shadow/no padding) but LEFT the filter on → it forces the card onto its own filter render-surface, and nested inside `.b2c-lang-panel`'s rounded `overflow:hidden` clip + the transform-composited `.animate-on-scroll` layer (`.visible` ends at `transform:translateY(0)`, a non-`none` transform → permanent GPU layer) Chrome double-rasterized the text. PROOF it's the filter, not a container clip: the right-side `.b2c-lang-grid` uses the SAME `overflow:hidden + border-radius:20px + isolation:isolate` recipe yet renders perfectly clean — it just isn't a `.card`. Fix: added `backdrop-filter:none; -webkit-backdrop-filter:none`.
  - (b) latest commit — user "把表单里的圆角改成直角,不要影响到控件和黄色卡片" → added `border-radius:0` to the SAME rule. Squares only the form-card wrapper `.lang-request-card`; the yellow `.b2c-lang-panel` frame and the inner `.form-input` / `.btn` controls keep their own radii. NOTE: this card is transparent/borderless/shadowless so its `border-radius` is visually invisible — (b) is a structural change with no visible effect; the actual ghosting fix is (a).
  - Revert: drop the `backdrop-filter:none` pair and/or the `border-radius:0` line from `.b2c-lang-aside .lang-request-card`. VERIFIED in served dev CSS (`/_next/static/chunks/src_app_*.css`). NOT screenshot-verified (autodebug needs the user to open the preview + enable debug mode).

  ───── EARLIER (concurrent threads, all committed & pushed `design-draft-local`): (1) `7431a0c` — b2c `#gallery` "Product Gallery": replaced the flat 4-phone `grid grid-4` row with an interactive 3D perspective phone cluster — new client component `src/app/[locale]/b2c/Gallery3D.tsx` (`.gallery-3d` `perspective:2000px` → `.gallery-3d-stage` `preserve-3d` → 4 `.g3d-phone` `.g3d-slot-{bl,fl,fr,br}`; mouse-parallax + hover-pop; `@media(max-width:760px)` flattens to a 2-col grid), `polish.css` `.gallery-3d`/`.g3d-*` block ~line 572, old `.gallery-grid`/`.gallery-phone`/`@keyframes gallery-float` removed. **THEN user "不满意，每个手机界面都添加个手机样机，交互不要黄色边缘线，标题胶囊去掉"** → reworked into a proper phone mockup: `.g3d-device` now `padding:4.2%` + deeper `border-radius:19%/9.2%` + 4-stop graphite gradient + a bright metallic rim drawn via `.g3d-device::before` (the `padding`+`mask`/`mask-composite:exclude` gradient-border trick); `.g3d-notch` darkened to `#08080a`; **removed the gold `0 0 0 3px rgba(254,191,29,.55)` hover/focus ring** (hover keeps only the bigger neutral shadow) and **deleted the `.g3d-label` caption pills entirely** (CSS rule + the `<span class="g3d-label">` in `Gallery3D.tsx`; `label` prop kept — still feeds the phone's `aria-label`). Slot widths bumped 17.5→18% / 20→20.5%. Revert: delete `Gallery3D.tsx`, restore old `grid grid-4 gallery-grid` block + `.gallery-*` CSS. (2) Pricing `/[locale]/pricing` comparison-table `<thead>` polish: `7765016` sticky header on ≥1040px viewports, `725253a` th `padding-bottom:33px`, `42b84c7` header text centred + table-wrap top corners squared, `c247b10` `功能` header cell left-aligned with the feature-name column.
